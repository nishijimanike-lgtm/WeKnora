export default {
  ...
  datasource: {
    ...
    localDir: {
      location: 'Расположение папки',
      rootPath: 'Путь к папке на сервере',
      rootPathPlaceholder: 'напр. /data/local-datasources/my-data',
      rootPathHint: 'Абсолютный путь к папке на сервере WeKnora. Должен находиться в пределах разрешённых администратором каталогов (WEKNORA_LOCAL_DATASOURCE_ALLOWED_ROOTS). Подпапки позволяют разделять данные разного характера или происхождения.',
      fileExtensions: 'Фильтр типов файлов (необязательно)',
      fileExtensionsPlaceholder: 'напр. pdf, docx, md',
      fileExtensionsHint: 'Через запятую. Оставьте пустым, чтобы синхронизировать все поддерживаемые форматы.',
      maxFileSize: 'Максимальный размер файла (МБ)',
      maxFileSizeHint: 'Файлы больше лимита пропускаются. По умолчанию: 100 МБ.',
      includeHidden: 'Включать скрытые файлы и папки (начинающиеся с .)',
      includeHiddenHint: 'По умолчанию скрытые файлы и папки пропускаются.',
      resourceHint: 'Выберите подпапки или файлы для синхронизации. Каждая подпапка может содержать данные разного характера или происхождения; структура папок сохраняется в базе знаний.'
    },
    ...
    noResources_local_dir: 'В папке нет содержимого для синхронизации',
    noResourcesDesc_local_dir: 'В папке нет поддерживаемых файлов или подпапок. Проверьте путь к папке или сначала добавьте файлы.',
    guideStep1_local_dir: 'Создайте в папке на сервере подпапки по характеру или происхождению данных и поместите туда поддерживаемые файлы',
    guideStep2_local_dir: 'Вернитесь на предыдущий шаг, чтобы изменить путь к папке, фильтр типов файлов и другие настройки',
    guideStep3_local_dir: 'Нажмите «Повторить», чтобы перезагрузить содержимое папки',
    ...
    connectorDesc: {
      ...
      local_dir: 'Синхронизация файлов из папки на сервере WeKnora; подпапки позволяют разделять данные разного характера или происхождения'
    },
    connector: {
      feishu: 'Feishu (Фэйшу)',
      lark: 'Lark',
  ...
  knowledgeBase: {
    ...
    channelLocalDir: 'Локальная папка',
